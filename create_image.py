...
BACKGROUND_IMAGE_PATH = "./create_work/dataset/background_images"
TARGET_IMAGE_PATH = "./create_work/dataset/king_slime_images"
OUTPUT_PATH = "./create_work/dataset/king_slime_dataset"

S3Bucket = "s3://king_slime_dataset"
...
# 新しいメタルキング画像取得クラス
class MetalKing:
    def __init__(self, king_path):
        self.__king_path = king_path

    def get(self):
        imagePath = random.choice(glob.glob(self.__king_path + "/*.png"))
...
class Target:
    def __init__(self, target_path, base_width, class_name):
        ...
        self.__metal_king = MetalKing(target_path)  # MetalKingクラスのインスタンス化

    def get(self, class_id):
        # 対象画像（メタルキング画像の取得）
        class_name = self.__class_name[class_id]
        target_image = self.__metal_king.get()
         # 元画像のアスペクト比を保持しつつ、背景画像のサイズに合わせてリサイズ
        h, w, _ = target_image.shape
        scale = min(BACK_WIDTH / w, BACK_HEIGHT / h)
...
class Transformer:
    ...
    def warp(self, target_image):
        ...
        # 配置位置決定
        max_x = max(self.__width - w, 0)
        max_y = max(self.__height - h, 0)
...
def main():
    ...
    target = Target(TARGET_IMAGE_PATH, BASE_WIDTH, CLASS_NAME)  # メタルキング画像のパスを指定
